## $\color{#fffff}{\text{Lab 3: Human Physiology - Homeostasis and Health}}$

$\color{#fffff}{\text{This project serves as a portfolio for visualising Spirometry data using R.}}$ <br>
### $\color{#fffff}{\text{📊 Experimental Results Summary}}$ <br>
### $\color{#fffff}{\text{📊 1. Normal vs Obstructed}}$ <br>

|Respiratory Parameter                           | Normal Breathing | Obstructed Breathing |
|:-----------------------------------------------|:----------------:|:--------------------:|
|Peak inspiratory flow (PIF), L/s                |      1.437       |        0.4128        |
|Peak expiratory flow (PEF), L/s                 |      -2.019      |       -0.3639        |
|Time for forced vital capacity (FVC), s         |      -4.280      |       -10.7500       |
|Forced vital capacity (FVC), L                  |      2.602       |        3.7830        |
|Forced expiratory volume in 1 second FEV<sub>1</sub>, L |      2.667       |        3.5270        |
|% FVC expired (in 1s), %                        |     102.500      |       93.2300        |

### $\color{#fffff}{\text{📊 2. Pulmonary function test}}$ <br>
|Description                                | Normal Breathing |
|:------------------------------------------|:---------------:|
|Respiratory rate (RR), breaths/min (BPM)   |     16.380      |
|Tidal volume (Vᴛ), L                       |      1.380      |
|Expired minute volume (Vᴇ(=Vᴛ*BPM)), L/min |     22.600      |
|Inspiratory reserve volume (IRV), L        |      2.821      |
|Expiratory reserve volume (ERV), L         |      1.073      |
|Sample's predicted RV (RV), L              |       NA        |

$\color{#fffff}{\text{Note. Below shows subtracted value: Sample's calculated value-Normal breathing}}$

|Lung Capacities (L)                       | Calculated Value-Normal Breathing |
|:-----------------------------------------|:---------------------------------:|
|Inspiratory capacity (IC=Vᴛ+IRV)          |               4.20                |
|Expiratory capacity (EC=Vᴛ+ERV)           |               2.45                |
|Vital capacity (VC=IRV+ERV+Vᴛ)            |               5.27                |
|Functional residual capacity (FRC=ERV+RV) |                NA                 |
|Total lung capacity (TLC=VC+RV)           |                NA                 |

### $\color{#fffff}{\text{🛠 Tools Used}}$
- $\color{#fffff}{\text{Language: R}}$
- $\color{#fffff}{\text{Libraries: knitr}}$
